#!/bin/bash
#SBATCH -J iSCALE_run_demo                   # Job name
#SBATCH -p gpu_xxx                           # Partition/queue name
#SBATCH -n 1                                 # Number of CPU cores
#SBATCH --gres=gpu:1                         # Request 1 GPU (adjust if cluster uses different syntax)
#SBATCH --mem=64G                            # Memory
#SBATCH --time=100:00:00                     # Walltime (100 hours)
#SBATCH -o logs/logs_output/hs_output_%J.log # Stdout
#SBATCH -e logs/logs_errors/hs_error_%J.log  # Stderr
